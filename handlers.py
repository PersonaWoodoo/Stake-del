...
from keyboards import (
    privacy_menu, subscribe_menu, captcha_menu, main_menu,
    snos_type_menu, confirm_snos_menu, back_menu, cancel_menu,
    buy_attempt_menu, admin_menu,
    broadcast_confirm_menu, broadcast_links_menu,
)
...
class AdminStates(StatesGroup):
    ...
    broadcast_link_name = State()
    broadcast_link_url = State()
...
async def cmd_start(message: Message, bot: Bot, state: FSMContext):
    ...
    if not user:
        await create_user(message.from_user.id, message.from_user.username or "", 0)
        await state.set_state(CaptchaStates.waiting)
        await message.answer(
            f"📋 <b>Условия использования</b>\n\n"
            f"Перед началом работы ознакомься с условиями конфиденциальности и правилами использования Бота.\n\n"
            f"⚠️ Использование Бота разрешено лицам <b>18+</b>.\n\n"
            f"Продолжая, ты подтверждаешь, что:\n"
            f"• Тебе исполнилось 18 лет\n"
            f"• Ты ознакомился с условиями\n"
            f"• Ты принимаешь их в полном объёме\n"
            f"• Бот создан в развлекательных целях\n\n"
...
async def accept_privacy(call: CallbackQuery, state: FSMContext, bot: Bot):
    await add_attempts(call.from_user.id, START_ATTEMPTS)

    data = await state.get_data()
    ref_id = data.get("ref_id")

    if ref_id and ref_id != call.from_user.id:
        await save_referral(ref_id, call.from_user.id)
...
async def refs_cb(cb: CallbackQuery, bot: Bot):
    ...
    text = (
        ...
        f"⚠️ Реферал засчитывается только после подписки на канал."
    )
...
async def admin_broadcast(cb: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(bc_text="", bc_links=[])
    await cb.message.edit_text(
        f"{E.BROADCAST} <b>Рассылка</b>\n\n"
        f"Шаг 1/2: Отправь <b>текст</b> для рассылки",
        parse_mode="HTML", reply_markup=cancel_menu()
    )
...
async def admin_broadcast_text(message: Message, state: FSMContext):
    ...
    await state.set_state(AdminStates.broadcast_link_name)
    await message.answer(
        f"✅ Текст сохранён\n\n"
        f"Шаг 2/2: Добавь inline-кнопки с ссылками.\n\n"
        f"Отправь название кнопки (например: <code>Перейти в бот</code>)\n"
        f"Или отправь <code>-</code> — рассылка без кнопок",
        parse_mode="HTML",
        reply_markup=broadcast_confirm_menu()
    )


@router.callback_query(F.data == "bc_add_link")
async def bc_add_link(cb: CallbackQuery, state: FSMContext):
    if not await is_admin_full(cb.from_user.id):
        return
    await state.set_state(AdminStates.broadcast_link_name)
    await cb.message.edit_text(
        f"🔗 <b>Добавить кнопку</b>\n\n"
        f"Отправь название кнопки (например: <code>Перейти в бот</code>)",
        parse_mode="HTML", reply_markup=cancel_menu()
    )


@router.message(AdminStates.broadcast_link_name)
async def bc_link_name(message: Message, state: FSMContext, bot: Bot):
    if not await is_admin_full(message.from_user.id):
        return
    ...
    if text == "-":
        data = await state.get_data()
        await send_broadcast(message, state, bot, data)
        return

    await state.update_data(bc_current_link_name=text)
    await state.set_state(AdminStates.broadcast_link_url)
    await message.answer(
        f"🔗 <b>Ссылка для кнопки</b>\n\n"
        f"Отправь URL (например: <code>https://t.me/stake_del</code>)",
        parse_mode="HTML", reply_markup=cancel_menu()
    )


@router.message(AdminStates.broadcast_link_url)
async def bc_link_url(message: Message, state: FSMContext, bot: Bot):
    if not await is_admin_full(message.from_user.id):
        return
    ...
    if not (url.startswith("http://") or url.startswith("https://")):
        await message.answer(
            f"{E.ERROR} Ссылка должна начинаться с http:// или https://",
            parse_mode="HTML", reply_markup=cancel_menu()
        )
        return

    data = await state.get_data()
    links = data.get("bc_links", [])
    name = data.get("bc_current_link_name", "Ссылка")

    links.append({"name": name, "url": url})
    await state.update_data(bc_links=links, bc_current_link_name="")
    ...
    await message.answer(
        ...
        f"Добавь ещё или нажми <b>✅ Отправить</b>",
        parse_mode="HTML",
        reply_markup=broadcast_links_menu()
    )
...
async def bc_send(cb: CallbackQuery, state: FSMContext, bot: Bot):
    if not await is_admin_full(cb.from_user.id):
        return
    data = await state.get_data()
    await send_broadcast(cb.message, state, bot, data, is_callback=True)
...
async def send_broadcast(message: Message, state: FSMContext, bot: Bot, data: dict, is_callback: bool = False):
    ...
    if links:
        rows = []
        for l in links:
            rows.append([InlineKeyboardButton(text=l["name"], url=l["url"], style="primary")])
    ...
    for u in users:
        try:
            await bot.send_message(u[0], text, parse_mode="HTML", reply_markup=kb)
            ok += 1
        except Exception:
            fail += 1
    ...
    result_text = (
        ...
        f"✅ Доставлено: <b>{ok}</b>\n"
        f"❌ Ошибок: <b>{fail}</b>\n"
        f"🔗 Кнопок: <b>{len(links)}</b>"
    )

    if is_callback:
        try:
            await message.edit_text(result_text, parse_mode="HTML", reply_markup=admin_menu())
        except Exception:
            await message.answer(result_text, parse_mode="HTML", reply_markup=admin_menu())
    else:
        await message.answer(result_text, parse_mode="HTML", reply_markup=admin_menu())
